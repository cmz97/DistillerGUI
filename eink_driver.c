...
static int FULL_REFRESH_CYCLE =
	30; // Number of flushes before doing a full refresh
...
static void save_pixels_as_png(const uint8_t *px_map, uint32_t width,
			       uint32_t height, const char *filename);
...
static void convert_2bit_to_1bit(const uint8_t *input, uint8_t *output,
				 size_t pixel_count)
{
	printf("Debug: Converting %zu pixels from 2-bit indexed to 1-bit\n",
	       pixel_count);
	memset(output, 0xFF,
	       (pixel_count + 7) / 8); // Initialize to white (0xFF)
...
static void convert_2bit_to_4gray(const uint8_t *input, uint8_t *output,
				  size_t pixel_count)
{
...
void EPD_init(void)
{
	...
	EPD_W21_WriteDATA(
		0x97); // WBmode:VBDF 17|D7 VBDW 97 VBDB 57  WBRmode:VBDF F7 VBDW 77 VBDB 37  VBDR B7
}
...
void EPD_init_Fast(void)
{
	...
	EPD_W21_WriteDATA(
		0x5F); // vendor uses 0x5F 1.5s, 0x5A 1.2s used for 4-gray mode
...
void PIC_display(const unsigned char *new_data)
{
	...
	for (i = 0; i < BUFFER_SIZE; i++) {
		EPD_W21_WriteDATA(
			old_data[i]); // Transfer the actual displayed data
	}

	EPD_W21_WriteCMD(0x13); // Transfer new data
	for (i = 0; i < BUFFER_SIZE; i++) {
		EPD_W21_WriteDATA(
			new_data[i]); // Transfer the actual displayed data
...
void pic_display_partial(uint32_t x_start, uint32_t y_start,
			 const uint8_t *data, uint32_t part_width,
			 uint32_t part_height)
{
	...
	size_t size =
		(part_width * part_height) / 8; // Bytes for the partial area
	...
	// Send old data
	static uint8_t old_partial_data
		[BUFFER_SIZE]; // Persistent buffer for partial updates
...
static void debug_pixel_data(const uint8_t *px_map, size_t size, uint32_t width,
			     uint32_t height)
{
	int black_count = 0;
	int white_count = 0;
	...
	printf("Black: %d (%.1f%%)\n", black_count,
	       (black_count * 100.0f) / total);
	printf("White: %d (%.1f%%)\n", white_count,
	       (white_count * 100.0f) / total);
...
static void flip_buffer_vertical(uint8_t *buffer, uint32_t width,
				 uint32_t height)
{
	const uint32_t bytes_per_row =
		(width + 7) / 8; // Round up to nearest byte
	...
	for (uint32_t i = 0; i < height / 2; i++) {
		...
		uint8_t *bottom_row =
			buffer + ((height - 1 - i) * bytes_per_row);
...
void eink_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
	...
	uint32_t size = (width * height) /
			8; // Size in bytes since it's 1-bit per pixel

	printf("Area: (%d,%d) to (%d,%d), size: %dx%d\n", area->x1, area->y1,
	       area->x2, area->y2, width, height);
...
const uint8_t LUT_ALL[216] = {
	,
};
...
static void spi_write_keep_cs(const uint8_t *data, size_t size)
{
	...
	while (remaining > 0) {
		size_t chunk_size = (remaining > MAX_CHUNK) ? MAX_CHUNK :
							      remaining;
		...
		struct spi_ioc_transfer tr = {
			...
			.bits_per_word = 8,
			.delay_usecs = 0,
			.cs_change = (remaining <= MAX_CHUNK) ?
					     1 :
					     0, // Release CS on last chunk
		};
			       ...
			       uint32_t height, const char *filename)
{
	...
	for (uint32_t y = 0; y < height; y++) {
		for (uint32_t x = 0; x < width; x++) {
			...
			uint8_t bit_pos =
				7 - (i % 8); // MSB first (7,6,5,4,3,2,1,0)
			uint8_t pixel = (px_map[byte_idx] >> bit_pos) & 0x01;
